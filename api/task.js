 ...
 * notes/updates, and system links. This is the DEPLOYED copy (Vercel builds
 * from the repo root); keep in sync with Transition/api/task.js.
 * Docs: Transition/TASK-API.md
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const task = await client.query("tasks:getTaskById", { taskId });
    ...
    if (req.method === "POST") {
      if (resource === "bug" || resource === "feature") {
        ...
        await client.mutation("tasks:addTaskFeature", {
          taskId,
          feature,
      ...
      if (resource === "note") {
        ...
        const notes = await client.mutation("tasks:addNoteToTask", {
          taskId,
    ...
    if (req.method === "PATCH") {
      if (resource === "feature" || resource === "bug") {
        ...
        if (body.name !== undefined || body.description !== undefined) {
          await client.mutation("tasks:updateTaskFeature", {
            taskId,
            featureId,
            updates: {
        ...
        if (body.status !== undefined) {
          ...
          await client.mutation("tasks:updateFeatureStatus", {
            taskId,
            featureId,
            status: body.status,
      ...
      if (resource === "links") {
        ...
        const links = await client.mutation("tasks:updateTaskLinks", {
          taskId,
    ...
    if (req.method === "DELETE") {
      ...
      await client.mutation("tasks:deleteTaskFeature", { taskId, featureId });
